...
#Plot UMAP (by timepoint)
data("hr.int")
umap <- hr.int[["umap"]]
dr <- FetchData(hr.int, vars = c("timepoint"))
df <- fortify(umap, data = dr)
timeumap <- ggplot(df, mapping = aes(x = umap_1, y = umap_2, color = timepoint)) +
  geom_point(alpha=.8, stroke=0, size = 1) +
  theme_classic(base_size = 14) +
  scale_color_brewer(palette="RdYlBu")
timeumap + DarkTheme()


#Plot tSNE (by timepoint)
tSNE.time <- DimPlot(hr.int,
                     reduction = "tsne",
                     group.by = "timepoint",
                     label = TRUE,
                     repel = TRUE,
...
#Compare UMAPs for each timepoint
DimPlot(hr.int, split.by = "timepoint", ncol = 3, group.by = "timepoint", pt.size = 0.4) & scale
